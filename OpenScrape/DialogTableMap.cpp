...
CDlgTableMap *p_DlgTableMap = NULL;
...
void CDlgTableMap::TraverseTreeBranch(CTreeCtrl* pTreeCtrl, HTREEITEM hItem, HTREEITEM hParentItem)
{
	is_collecting_fonts = true;
	m_TableMapTree.Select(hItem, TVGN_CARET);
	if (m_CreateFont.IsWindowEnabled()){
		SendMessage(WM_COMMAND, MAKEWPARAM(IDC_CREATE_FONT, BN_CLICKED));
	}

	HTREEITEM hItemTmp = pTreeCtrl->GetChildItem(hItem);
	HTREEITEM hNewItem = pTreeCtrl->GetParentItem(hItemTmp);
	while (hItemTmp != NULL)
	{
		if (abort_font_collection)
			break;
		TraverseTreeBranch(pTreeCtrl, hItemTmp, hNewItem);
		hItemTmp = m_TableMapTree.GetNextSiblingItem(hItemTmp);
	}
}

void CDlgTableMap::CollectFonts()
{	
	HTREEITEM	hItem = m_TableMapTree.GetRootItem();
	...
	while (bContinue && hItem)
	{
		...
		if (!bContinue) {
			is_collecting_fonts = false;
			MessageBox("Font collection aborted.", "Info");
		}
		else {
			if (m_TableMapTree.GetItemText(hItem) == "Regions") {
				TraverseTreeBranch(&m_TableMapTree, hItem, TVI_ROOT);
				if (abort_font_collection) {
					is_collecting_fonts = false;
					abort_font_collection = false;
					MessageBox("Font collection aborted.", "Info");
				} else {
					is_collecting_fonts = false;
					MessageBox("Font collection finished.", "Info");
				}
				//m_WaitDlg.Close();
				break;
			}
			else
				hItem = m_TableMapTree.GetNextItem(hItem, TVGN_NEXT);
		}
	}
}