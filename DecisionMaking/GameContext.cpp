...
namespace G5Decision
{
    namespace
    {
        ...
        void SortStrengthPairs(StrengthSortPair* pairs, int nPairs, const AllCounters& counters, const bool* isBoardCard)
        {
            ...
            for (int i = 0; i < 51; i++) // First card in hand
            {
                for (int j = i + 1; j < 52; j++) // Second card in hand
                {
    ...
    void GameContext::sortHoleCards(float* flopEquities, int* flopCounts, int tStart, int tEnd)
    {
        ...
        for (int i = tStart; i < tEnd; i++) // Possible turns
        {
            if (isBoardCard[i])
                continue;
            ...
            for (int j = 0; j < 52; j++) // Possible rivers
            {
                if (isBoardCard[j])
                    continue;
