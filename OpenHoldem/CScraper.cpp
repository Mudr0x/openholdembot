...
void CScraper::ScrapeActing() {
	...
	//   * as people might be missing in tournament, but we use ICM
		...
		{*/
			...
			CString acting;
			CString result;
			// We must NOT set_acting(false) here,
			// as that would reset all player data.
			// http://www.maxinmontreal.com/forums/viewtopic.php?f=156&t=20567
			acting.Format("p%dacting", chair);
			if (EvaluateRegion(acting, &result)) {
				if (result != "") {
					p_table_state->Player(chair)->set_acting(p_string_match->IsStringActing(result));
				}
			}
			if (p_table_state->Player(chair)->acting()) {
				_acted[chair] = true;
				return;
			}
			// try u region next uXacted,
			// but only if we didn't get a positive result from the p region
			acting.Format("u%dacting", chair);
			if (EvaluateRegion(acting, &result)) {
				if (result != "") {
					p_table_state->Player(chair)->set_acting(p_string_match->IsStringActing(result));
				}
			...
			if (_acted[chair] && !p_table_state->Player(chair)->acting()) {
				_acted[chair] = false;
				ScrapeAction(chair);
				return;
			}
			...
			// Failed. Not acted
			//p_table_state->Player(chair)->set_acting(false);
		//}
	}
}

void CScraper::ScrapeAction(int chair) {
	CString action;
	CString result;
	// We must NOT set_action(false) here,
	// as that would reset all player data.
	// http://www.maxinmontreal.com/forums/viewtopic.php?f=156&t=20567
	action.Format("p%daction", chair);
	if (EvaluateRegion(action, &result)) {
		p_table_state->Player(chair)->set_action(result.MakeLower());
	}
}
